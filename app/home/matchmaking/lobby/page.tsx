...
export default function Lobby (): React.ReactElement {
  ...
  const handleReadyUp = async (): Promise<void> => {
    socketRef.current?.emit('message', { room: `${lobbyRoomId}`, action: 'player ready', message: 'ready' });
    setReady(true)
  }
        ...
        <div className='px-6 py-4'>
            ...
            <div className="flex flex-col">
              <div className={`flex flex-row p-2 items-center bg-gray-900 border-[1.5px] ${opponentReady ? 'border-green-600' : 'border-gray-700'} rounded-xl`}>
                {opponentAvatar !== null && <img className="h-[50px] rounded-xl" src={`/${opponentAvatar}`}></img>}
                <div className="flex flex-col ml-6">
                {opponentUsername !== '' && opponentUsername !== null
                  ? <div className='text-xl font-semibold'>{opponentUsername}</div>
                  : <Skeleton baseColor="#202020" highlightColor='#808080'></Skeleton>}
                {opponentLanguage !== '' && opponentLanguage !== null
                  ? <div className='text-xs font-light'>{opponentLanguage.toUpperCase()}</div>
                  : <Skeleton baseColor="#202020" highlightColor='#808080'></Skeleton>}
                </div>
              </div>
            </div>
            ...
            {!ready
              ? <button onClick={() => { handleReadyUp().catch(console.error) }} className="bg-orange-500 mt-12 py-2 px-32 hover:bg-orange-600 text-white font-bold text-lg rounded-lg">{'I\'m Ready'}</button>
              : <div className="bg-gray-700 border-[1px] border-gray-600 mt-12 py-2 px-32 font-bold text-lg text-green-400 rounded-lg">{'I\'m Ready'}</div>
            }
          </div>
      </div>
      </div>
